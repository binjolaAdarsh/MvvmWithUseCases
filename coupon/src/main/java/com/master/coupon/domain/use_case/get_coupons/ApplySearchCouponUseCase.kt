...
import com.master.coupon.domain.model.ApplyDataResponse.Companion.asSearchDataResponse
import com.master.coupon.domain.model.ApplySearchDataResponse
...
@ViewModelScoped
class ApplySearchCouponUseCase @Inject constructor(
        ...
        try {
            ...
            when {
                couponsResponse.success -> {
                    ...
                    }?.asSearchDataResponse() ?: ApplySearchDataResponse("")
                    ...
                    emit(Either.Left(CouponError.CouponSearchApplyFailError(couponsResponse.message)))
            }
        } catch (e: HttpException) {
            emit(Either.Right(Success.Loading(false)))
            emit(Either.Left(CouponError.CouponSearchApplyFailError(SERVER_GENERIC_APPLY_ERROR)))
        } catch (e: IOException) {
            emit(Either.Right(Success.Loading(false)))
